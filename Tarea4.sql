...
select dp.customer_id, \
	   ...
	   (date_part('day', avg(dp.delta_pago))||' d\'edas')||' '||(date_part('hour',avg(dp.delta_pago))||' horas') as "Tiempo promedio entre pagos"\
from dif_pagos dp\
...
\
select *, \
(date_part('day', avg_delta_pagos-avg_delta_rentas)||' d\'edas'||' '||(date_part('hour',avg_delta_pagos-avg_delta_rentas)||' horas'))\
